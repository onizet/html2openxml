...
namespace NotesFor.HtmlToOpenXml
{
	...
	public class HtmlConverter
	{
		...
		/// <summary>Holds the tables in the order we discover them (to support nested tables).</summary>
		private Stack<HtmlDocumentStyle> tables;
		...
		public IList<OpenXmlCompositeElement> Parse(String html)
		{
			...
			htmlStyles.Tables.Reset();
			currentTable = null;
			currentParagraph = null;
			numberLevelRef = 0;

			// Start a new processing
			paragraphs.Add(currentParagraph = new Paragraph());
		...
		private void ProcessHtmlChunks(HtmlEnumerator en, String endTag)
		{
			while (en.MoveUntilMatch(endTag))
			{
				...
				else
				{
					...
					Run run = new Run(
						new Text(HttpUtility.HtmlDecode(en.Current)) { Space = "preserve" }
					);
					...
					new Footnote(
						new Paragraph(
							new ParagraphProperties(
								new SpacingBetweenLines() { After = (UInt32Value)0U, Line = 240, LineRule = LineSpacingRuleValues.Auto }),
							new Run(
								new SeparatorMark())
						)
				...
				new Footnote(
					new Paragraph(
						...
						new Run(
							...
							new Text(" " + description) { Space = "preserve" })
					)
				) { Id = footnotesRef });
		...
		private bool ProcessContainerAttributes(HtmlEnumerator en, IList<OpenXmlElement> styleAttributes)
		{
			...
			htmlStyles.Runs.ProcessCommonAttributes(en, styleAttributes);
					...
					new AbstractNum(
						...
						new Level(
							...
							new PreviousParagraphProperties(
								new Indentation() { Left = 420, Hanging = (UInt32Value)360U })
						) { LevelIndex = 0 }
					) { AbstractNumberId = 0 },
					new AbstractNum(
						new MultiLevelType() { Val = MultiLevelValues.SingleLevel },
		...
		private void ProcessDefinitionList(HtmlEnumerator en)
		{
			ProcessParagraph(en);
			currentParagraph.InsertInProperties(
				 new SpacingBetweenLines(){ After = (UInt32Value)0U });
		}
		...
		private void ProcessDefinitionListItem(HtmlEnumerator en)
		{
			...
			currentParagraph = new Paragraph(elements);
			currentParagraph.InsertInProperties(
				   new Indentation() { FirstLine = (UInt32Value)708U },
				   new SpacingBetweenLines(){ After = (UInt32Value)0U }
			);
		...
		private void ProcessFont(HtmlEnumerator en)
		{
			...
			if (attrValue != null)
			{
				...
				if (fontSize != 0L)
					styleAttributes.Add(new FontSize { Val = fontSize });
			}
		...
		private void ProcessHeading(HtmlEnumerator en)
		{
			...
			this.paragraphs.Add(currentParagraph = new Paragraph());
		}
		...
		private void ProcessLink(HtmlEnumerator en)
		{
			...
			if (!String.IsNullOrEmpty(att))
			{
				...
				{
					ExternalRelationship extLink =
						mainPart.AddExternalRelationship(KnownRelationshipType.Hyperlink, uri);
		...
		private void ProcessTable(HtmlEnumerator en)
		{
			...
			if (unit.IsValid)
			{
				...
				{
					case "%":
						properties.Add(new TableWidth() { Type = TableWidthUnitValues.Pct, Width = unit.Value * 50 }); break;
					case "pt":
						properties.Add(new TableWidth() { Type = TableWidthUnitValues.Dxa, Width = unit.Value * 20 }); break;
					case "px":
						properties.Add(new TableWidth() { Type = TableWidthUnitValues.Dxa, Width = unit.Value }); break;
				}
			}
			else
			{
				properties.Add(new TableWidth() { Type = TableWidthUnitValues.Pct, Width = 100 *50 });
			}
			...
			if (align != null)
			{
				...
				if(halign.HasValue)
					properties.Add(new TableJustification(){ Val = (TableRowAlignmentValues) halign });
			}
			...
			if (currentTable != null)
			{
				...
				elements.Add(currentTable = new Table(
			...
			htmlStyles.Tables.CellPosition = Point.Empty;
			htmlStyles.Tables.RowSpan.Clear();
		}
		...
		private void ProcessTableCaption(HtmlEnumerator en)
		{
			...
			var legend = new Paragraph(
					...
					new Run(
						new RunProperties(
							new RunStyle() { Val = runStyleId }),
						new FieldCode(" SEQ Tableau \\* ARABIC ") { Space = "preserve" }),
					new Run(
						new RunProperties(
							new RunStyle() { Val = runStyleId }),
			...
			else
			{
				...
				TableProperties props = currentTable.GetFirstChild<TableProperties>();
				if (props != null)
				{
					TableJustification justif = props.GetFirstChild<TableJustification>();
					if (justif != null) legend.InsertInProperties(new Justification { Val = (JustificationValues)justif.Val.Value });
				}
			}
		...
		{
			this.currentTable.Append(this.elements);
			this.elements.Clear();

			List<OpenXmlElement> styleAttributes = new List<OpenXmlElement>();
			htmlStyles.Tables.ProcessCommonAttributes(en, styleAttributes);

			ProcessHtmlChunks(en, "</tr>");

			TableRow row = new TableRow();
			if(styleAttributes.Count > 0)
				row.Append(new TableRowProperties(styleAttributes));

			int rowIndex = htmlStyles.Tables.CellPosition.Y;

			if(htmlStyles.Tables.RowSpan.Count > 0)
			{
				for (int i = 0; i<htmlStyles.Tables.RowSpan.Count; i++)
				{
					Point position = htmlStyles.Tables.RowSpan.Keys[i];
					if (position.Y == rowIndex) continue;

					elements.Insert(position.X, new TableCell(
							new TableCellProperties(
								new VerticalMerge()),
							new Paragraph())
					);

					int span = htmlStyles.Tables.RowSpan[position];
					if (span == 1) { htmlStyles.Tables.RowSpan.RemoveAt(i); i--; }
					else htmlStyles.Tables.RowSpan[position] = span - 1;
				}
			}
			...
			htmlStyles.Tables.CellPosition = new Point(0, rowIndex + 1);
		}
		...
		private void ProcessTableColumn(HtmlEnumerator en)
		{
			...
			if (unit.IsValid)
			{
				...
				{
					case "%":
						styleAttributes.Add(new TableCellWidth() { Type = TableWidthUnitValues.Pct, Width = unit.Value * 50 });
						break;
					case "pt":
						styleAttributes.Add(new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = unit.Value * 20 });
						break;
					case "px":
						styleAttributes.Add(new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = unit.Value });
						break;
				}
			}
			...
			if (rowspan.HasValue)
			{
				...
				htmlStyles.Tables.RowSpan[htmlStyles.Tables.CellPosition] = rowspan.Value - 1;
			}
			...
			var savedElements = this.elements;
			this.elements = new List<OpenXmlElement>();

			if(!en.IsSelfClosedTag) ProcessHtmlChunks(en, "</td>");
			...
			savedElements.Add(cell);
			this.elements = savedElements;
			htmlStyles.Tables.EndTagForParagraph("<td>");

			Point pos = htmlStyles.Tables.CellPosition;
			pos.X++;
			htmlStyles.Tables.CellPosition = pos;
		}
		...
		private void ProcessClosingTable(HtmlEnumerator en)
		{
			...
			TableRow row = currentTable.GetFirstChild<TableRow>();
			...
			currentTable.PrependChild<TableGrid>(grid);
